...
class SerialSocket extends BluetoothGattCallback {
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        for (BluetoothGattService gattService : gatt.getServices()) {
            ...
            if (gattService.getUuid().equals(BLUETOOTH_LE_MICROCHIP_SERVICE))                delegate = new MicrochipDelegate();
            if (gattService.getUuid().equals(BLUETOOTH_LE_NRF_SERVICE))                delegate = new NrfDelegate();
            if (gattService.getUuid().equals(BLUETOOTH_LE_TIO_SERVICE))                delegate = new TelitDelegate();
